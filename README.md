...
PostgreSQL 16 (Alpine) and Valkey are supplied for local development through [docker-compose.yml](docker-compose.yml). Use `make up` and `make down` as shortcuts to start and stop local infrastructure. FastAPI verifies the PostgreSQL connection during startup and releases its pool during shutdown. Valkey is used for authentication rate limits, with an explicit in-memory fallback available only for local development and tests. The project also includes LangGraph's PostgreSQL checkpoint integration for the forthcoming Agent runtime.
